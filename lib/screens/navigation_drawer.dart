...
import 'package:technoshop/screens/preRegister.dart';
...
//    NavMenuItem("Register", () => FacebookFeeds() ),
  ...
  Future _checkToken() async {
    ...
    token = sharedPreferences.get('token');
     secteur = sharedPreferences.get('secteur');
     print(secteur);
    setState(() {
      if (token == null) {
        isLoggedIn = false;
  ...
  void initState() {
    ...
    if (isLoggedIn) {
      if(isSte)
      {
       ...
       navigationMenu.add(
        NavMenuItem("Publish an offer", () => AddOffre()),
      );
      }
      else{
         navigationMenu.add(
        NavMenuItem("See all offres", () => HomePage()),
      );
      ...
      //  name
      navigationMenu.add(NavMenuItem("My favorites", () => CartItems()));
      iconsMenu.add(
        Icon(
          Icons.favorite,
          color: kSecondaryColor,
   ...
   navigationMenu.add(NavMenuItem("Logout", _logout));
     iconsMenu.add(
      ...
      navigationMenu.add(NavMenuItem("Register", () =>PreRegister()));
      navigationMenu.add(
