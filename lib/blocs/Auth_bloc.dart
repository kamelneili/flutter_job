...
class AuthBloc extends Bloc<AuthEvents, AuthState> {
  ...
  Stream<AuthState> mapEventToState(AuthEvents event) async* {
    ...
    if (event is StartEvent) {
      ...
      if (data['success'] == 1) {
        ...
        int userId = pref.getInt('id');
                        pref.setString("secteur", user.secteur);
    ...
    else if (event is RegisterSteButtonPressed) {
      
      yield RegisterLoadingState();
      var data = await repo.register(
          event.phone, event.name, event.email, event.password,
          event.adr,event.description,event.secteur,event.siteWeb);
       print(data);
      ...
      pref.setString("secteur", data['data']['secteur']);
      print(pref.getString("secteur"));
      // pref.setString("email",data['email']);
      yield UserRegisterSuccessState();
